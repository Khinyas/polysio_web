...
public class ViewPlateau extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		int tempsRestantSec = (session.getAttribute("tempsInitial") != null)
				? (int) session.getAttribute("tempsInitial") : 1800;

		// ON ÉCHAPPE LES % POUR ÉVITER LE CONFLIT AVEC formatted()
		String plateauHTML    = plateau.initialiserPlateau(joueur).replace("%", "%%");
		String inventaireHTML = plateau.genererInventaireHTML(joueur).replace("%", "%%");
		String listeJoueursFinal = listeJoueursHTML.toString().replace("%", "%%");

		String chronoHTML = """
		<div id="chrono-container" style="background:#333;color:#ffeb3b;padding:10px;
		    ...
		    <style>
		        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
		        ...
		        .game-area {
		            ...
		            justify-content: flex-start; gap: 16px; padding: 8px;
		            min-height: 0; overflow: hidden;
		        }
		        .jeu-container { width: 100%%; height: 100%%; }
		        .plateau-wrapper {
		            flex-shrink: 0; min-height: 0;
		            height: 100%%; aspect-ratio: 1 / 1; overflow: hidden;
		        }
		        .plateau-grid {
		            display: grid;
		        ...
		        .modal-overlay:target { display: flex; justify-content: center; align-items: center; }
		        .btn {
		            display: inline-block; padding: 8px 16px;
		            background: #4a7c59; color: white; border-radius: 4px; text-decoration: none;
		        }
		    </style>
		</head>
		<body>
